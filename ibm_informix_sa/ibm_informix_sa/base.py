...
class DB2Compiler(compiler.SQLCompiler):
    ...
    def visit_select(self, select, asfrom=False, parens=True, **kw):
        ...
        if asfrom and parens and self.dialect.server_version_info < (11,):
            return "table(multiset{0})".format(text)
        else:
            return text
        # below is for DB2 only not for INFORMIX
    ...
    def default_from(self):
        ...
        # not needed for Informix. need tofigure out how to tell what db we are hitting
        # perhaps if schema is informix do not use the below
    ...
    def visit_function(self, func, result_map=None, **kwargs):
        ...
        elif func.name.upper() == "CURRENT_DATE":
            return "today"
        elif func.name.upper() == "CURRENT_TIME":
            return "CURRENT HOUR TO SECOND"
        elif func.name.upper() in ("current_timestamp", "now"):
    ...
    def get_select_precolumns(self, select):
        s = ""
        if select._offset:
            s += "SKIP {0} ",format(select._offset)
        if select._limit:
            s += "FIRST {0} ".format(select._limit)
        s += select._distinct and "DISTINCT " or ""
        return s
        """The below is for db2 not for informix"""
